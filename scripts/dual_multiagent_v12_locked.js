...
// ===== Dual Multi‑Agent Plug‑in — v12 (HARD‑LOCKED model) =====
// Este build é igual ao v12 (12 arquétipos), mas *ignora* Brain/LS para o modelo.
// Todas as chamadas do Horus/arquétipos usarão SEMPRE o MODEL_SLUG abaixo.
// Para trocar o modelo, edite a constante MODEL_SLUG e re‑carregue o app.

(function(){
  // >>>>>>>>>>>>>>>> EDITE AQUI SE PRECISAR TROCAR O SLUG FREE <<<<<<<<<<<<<<
  const MODEL_SLUG = "meta-llama/llama-3.1-8b-instruct:free"; // ajuste se seu slug free for outro
  // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

  const $ = (q,r=document)=>r.querySelector(q);
  const LS = { get:(k,d)=>{ try{ const v=localStorage.getItem(k); return v?JSON.parse(v):d }catch{ return d } } };

  // UI styles (resumo, idêntico ao v12)
  const CSS = `#chatFeed .msg.horus{align-self:flex-start;background:rgba(255,195,0,.16);border-radius:16px 16px 16px 0;padding:8px 12px;max-width:75%}
  ...
  (function(){ if(!document.getElementById('multiagent12-locked-styles')){ const s=document.createElement('style'); s.id='multiagent12-locked-styles'; s.textContent=CSS; document.head.appendChild(s);} })();
  ...
  async function callOpenRouter(messages, { max_tokens=800, temperature=0.6 }={}){
    ...
    // Hard lock: sempre usa MODEL_SLUG
    const payload = { model: MODEL_SLUG, messages, max_tokens, temperature };
    if (!/:free\b/i.test(MODEL_SLUG)) {
      try { if (typeof showArchMessage==='function') showArchMessage('HARD‑LOCK: modelo não parece free. Edite MODEL_SLUG.', 'warn'); } catch {}
    }
  ...
  function shouldMulti(s){ const t=String(s||'').toLowerCase(); return /^@\s*horus\b/.test(t) || /\bhorus\b/.test(t) }
  function wantsAll(t){ const s=(t||'').toLowerCase(); return /\bos\s*doze\b|\btodos\b|\b12\b/.test(s) }

  async function runFlow(userText){
    const name = getUserName();
    const sk = getKey();
    if(!sk){ try{ showArchMessage('Configure chave no Brain.', 'warn') }catch{}; if (typeof window.__sendUserMessageSingle==='function') return window.__sendUserMessageSingle(userText); return; }
    ...
    agentPush('horus', `Perfeito, ${name}. Para elaborar seu plano, vou ativar ${ativ.map(a=>actorLabel(a)).join(', ')}.`);
  ...
  // Hook: só roda o multi quando mensagem mencionar Horus
  if (typeof window.sendUserMessage === 'function') {
    ...
    window.sendUserMessage = function(message){
      ...
      if (/^@\s*horus\b/i.test(msg) || /\bhorus\b/i.test(msg)) return runFlow(msg);
